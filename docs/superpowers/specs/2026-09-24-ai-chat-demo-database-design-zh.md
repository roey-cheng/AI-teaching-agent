...
**状态（更新于 2026-09-25）：** 五张表的 ORM 模型和 Alembic 初始迁移已通过离线检查与隔离 MySQL 8.4.11 的真实验收；临时测试资源已清理，用户随后报告已手动完成项目数据库建表迁移。
...
实施顺序（2026-09-25 调整）：SQLAlchemy 表模型完成、API Pydantic Schema 完成大部分后，按用户安排先完成初始迁移的编写与隔离 MySQL 验收；用户报告项目库建表成功后，再补齐记忆和通用错误 Schema。首份迁移为 `backend/alembic/versions/20260925_0001_create_chat_tables.py`，固定记录当前五张表结构。Alembic 自己还会创建 alembic_version 来记录迁移版本；它是工具的管理表，不算第六张业务表。具体命令和风险见[后端迁移说明](../../../backend/README.md#alembic-初始迁移)。
...
**当前已完成表模型及迁移脚本的离线验证，以及临时 MySQL 8.4.11 的建表、真实约束、DML 回滚、重复升级保留数据、回退再升级验收（脚本为 `backend/scripts/check_migration_mysql.py`）。隔离验收结束时项目库为空；用户随后报告已手动完成正式建表。本次补 Schema 未连接或修改数据库。运行期的失败重试、迟到保护等还须业务层完成后验证。**
